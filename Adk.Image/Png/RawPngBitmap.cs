...
using EndOfStreamException = Adk.Compression.Exceptions.EndOfStreamException;
...
namespace Adk.Image.Png
{
    ...
    public sealed class RawPngBitmap
    {
        ...
        public static RawPngBitmap Load(Stream input)
        {
            ...
            return PngDecoder.Load(
                new StreamPngInput(
                    input));
        }

        public static RawPngBitmap Load(byte[] input)
        {
            if (input == null)
                throw new ArgumentNullException(nameof(input));

            return PngDecoder.Load(
                new ByteArrayPngInput(
                    input));
        }
    ...
    sealed class StreamPngInput : IPngInput
    {
        readonly Stream _stream;

        public StreamPngInput(Stream stream)
        {
            _stream = stream;
        }
    ...
    sealed class ByteArrayPngInput : IPngInput
    {
        readonly byte[] _data;
        int _position;

        public ByteArrayPngInput(byte[] data)
        {
            _data = data;
        }
    ...
    internal static class PngDecoder
    {
        ...
        public static RawPngBitmap Load(IPngInput file)
        {
        ...
        static uint ReadUInt32BigEndian(IPngInput stream)
        {
        ...
        static void ReadExactly(IPngInput stream, byte[] buffer, int offset, int count)
        {
            while (count > 0)
            {
